                ...
                <div className="w-full">
                  ...
                  {thisLister.services.map((service, index) => (
                    <div key={index} className="text-sm md:text-md xl:text-lg pb-3 md:pb-0">
                      <div className={`${selectedServices.includes(service.name) ? 'bg-blue-500 text-white p-1 rounded-md' : 'bg-none'} flex justify-between min-w-[120px]`}>
                        <button className="cursor-pointer font-bold" onClick={() => handleServiceSelection(service.name)}>
                          {service.name}
                        </button>
                        {service.price && (
                      ...
                      {service.subcategories.length > 1 ? 
                        <p className="font-light opacity-50">Addon services for {service.name}</p> : ""
                      }
                      {/* question mark because they may not exists */}
                      {service.subcategories?.map((subService, jndex) => (
                        <div key={jndex} className={`${selectedServices.includes(subService.name) ? 'bg-blue-500 text-white p-1 rounded-md' : 'bg-none'} mt-1 md:mt-0 ml-3 flex flex-row justify-between font-light`} >
